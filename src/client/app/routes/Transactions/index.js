...
class TransactionsListing extends Component {
	...
	handlePaginationNext(e) {
		e.preventDefault();
		const { transactions: { page, limit, length }, triggerFetchTransactions } = this.props;
		toast.dismiss();
		if (length < limit) {
			// no more data
			toast.info('No more data');
		} else {
			triggerFetchTransactions(page+1, limit)
		}
	}
	...
	handlePaginationBack(e){
		e.preventDefault();
		toast.dismiss();
		const { transactions: { length, page, limit }, triggerFetchTransactions } = this.props;
		if (page > 1) {
			triggerFetchTransactions(page-1, limit);
		} else {
